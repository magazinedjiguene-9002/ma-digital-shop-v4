...
async function loadProducts(){
  try{
    ...
    const {data,error}=await window.supabaseClient.from("products").select("id,slug,name,subtitle,category,badge,price,promo_price,promo_active,promo_start,promo_end,image_url,description,available,featured,sort_order,icon").eq("available",true).order("sort_order",{ascending:true});
    if(error)throw error;
